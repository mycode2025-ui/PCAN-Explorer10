; PCAN-Explorer10 闁诲海鎳撻ˇ鎶剿夋繝鍥х闁告稒婢樻竟鏍煛?(Inno Setup 6)
...
AppName=PCAN-Explorer10
AppVersion={#AppVer}
AppPublisher=XCharge
VersionInfoVersion={#AppVer}
VersionInfoProductVersion={#AppVer}
VersionInfoCompany=XCharge
VersionInfoDescription=PCAN-Explorer10 CAN, Serial and Modbus Engineering Tools
VersionInfoProductName=PCAN-Explorer10
DefaultDirName={autopf}\PCAN-Explorer10
DefaultGroupName=PCAN-Explorer10
...
OutputBaseFilename=PCAN-Explorer10-Setup-{#AppVer}
SetupIconFile={#Root}\app.ico
UninstallDisplayIcon={app}\PCAN-Explorer10.exe
#ifdef FastPackage
...
SetupLogging=yes
; PCAN-Explorer10 owns the upgrade shutdown sequence below. Restart Manager cannot
...
#ifdef FastPackage
Source: "{#Root}\target\release-fast\PCAN-Explorer10.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "{#Root}\target\release-fast\PCAN-Explorer10.exe.integrity"; DestDir: "{app}"; Flags: ignoreversion
...
#else
Source: "{#Root}\target\release\PCAN-Explorer10.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "{#Root}\target\release\PCAN-Explorer10.exe.integrity"; DestDir: "{app}"; Flags: ignoreversion
...
[InstallDelete]
; The AppId is unchanged so upgrades can reuse an existing install directory.
; Remove the former main executable after stopping it to avoid two product names.
Type: files; Name: "{app}\pcanwork.exe"
Type: files; Name: "{app}\pcanwork.exe.integrity"

[Registry]
Root: HKCR; Subkey: ".pcprj"; ValueType: string; ValueName: ""; ValueData: "PCAN-Explorer10.Project"; Flags: uninsdeletevalue
...
Root: HKCR; Subkey: "PCAN-Explorer10.Project"; ValueType: string; ValueName: ""; ValueData: "PCAN-Explorer10 Project"; Flags: uninsdeletekey
Root: HKCR; Subkey: "PCAN-Explorer10.Project\DefaultIcon"; ValueType: string; ValueName: ""; ValueData: "{app}\project.ico,0"
Root: HKCR; Subkey: "PCAN-Explorer10.Project\shell\open\command"; ValueType: string; ValueName: ""; ValueData: """{app}\PCAN-Explorer10.exe"" ""%1"""

[Icons]
Name: "{group}\PCAN-Explorer10"; Filename: "{app}\PCAN-Explorer10.exe"; WorkingDir: "{app}"; IconFilename: "{app}\app.ico"
...
Name: "{group}\卸载 PCAN-Explorer10"; Filename: "{uninstallexe}"
Name: "{autodesktop}\PCAN-Explorer10"; Filename: "{app}\PCAN-Explorer10.exe"; WorkingDir: "{app}"; IconFilename: "{app}\app.ico"; Tasks: desktopicon
...
Filename: "{app}\PCAN-Explorer10.exe"; Description: "{cm:LaunchProgram,PCAN-Explorer10}"; Flags: nowait postinstall skipifsilent
...
var
  ...
  UninstallKey: String;
begin
  UninstallKey := 'Software\Microsoft\Windows\CurrentVersion\Uninstall\{2BD0E569-4F8D-4B31-A43A-5332CE87A30A}_is1';
  if RegQueryStringValue(HKLM64, UninstallKey, 'DisplayName', DisplayName) and
     (Pos('PcanWork', DisplayName) > 0) and
     (Pos('PCAN-Explorer10', DisplayName) = 0) and
     RegQueryStringValue(HKLM64, UninstallKey, 'UninstallString', Uninstaller) then
  begin
    if (Length(Uninstaller) >= 2) and (Uninstaller[1] = '"') then
    begin
      Delete(Uninstaller, 1, 1);
      if (Length(Uninstaller) > 0) and (Uninstaller[Length(Uninstaller)] = '"') then
        Delete(Uninstaller, Length(Uninstaller), 1);
    end;
    if FileExists(Uninstaller) then
    begin
      LegacyDir := ExtractFileDir(Uninstaller);
      if not Exec(Uninstaller, '/VERYSILENT /SUPPRESSMSGBOXES /NORESTART', '', SW_HIDE,
        ewWaitUntilTerminated, ResultCode) or (ResultCode <> 0) then
        RaiseException(Format('Unable to remove legacy PcanWork installation (exit code %d).', [ResultCode]));
...
begin
  ...
  StopProductProcess('PCAN-Explorer10.exe');
